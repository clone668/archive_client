...
- 生成逐对象 JSON 验证报告；“报告”页面可直接查看报告摘要、逐对象明细和
  清理报告字段，并可打开选中的原始报告文件或报告目录。
...
- 启用 Windows 计划任务后会立即启动一次补齐，之后每 30 分钟依次检查所有
  启用配置的前一个 UTC 日；客户端启动并完成首次清单读取后也会立即检查一次，
  本地缺失时自动下载，已验证日期不会重复下载。
- 下载和校验支持安全取消；正常关闭客户端会先停止传输并等待后台线程退出。
  同一本地归档根目录使用跨进程互斥，避免 UI 与计划任务同时写入。
- 双击已完成日期可打开其本地目录；本地归档栏可打开归档根目录，报告页面可
  打开原始报告文件与报告目录。
